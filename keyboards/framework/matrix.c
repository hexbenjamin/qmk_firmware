...
bool matrix_scan_custom(matrix_row_t current_matrix[]) {
    ...
    for (int col = 0; col < MATRIX_COLS; col++) {
        ...
        for (int row = 0; row < MATRIX_ROWS; row++) {
            ...
            // Let the analog mux output settle before sampling. Without this,
            // voltage from the previously selected row can leak into the ADC
            // reading and cause same-column keys on adjacent rows to ghost.
            wait_us(10);
